...
# Role IDs allowed to use staff tools
...
# =========================================================
# DB helper — read-only, no dependency on bot module
# =========================================================
...
class _FinalizeModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        import bot as _b
        import re
        raw = str(self.attendees) if self.attendees.value is None else self.attendees.value or ""
        user_ids = sorted({int(m) for m in re.findall(r"\d{15,25}", raw)})
        _b._rc_db.set_actual_attendees(interaction.guild.id, self.meet_number, user_ids)
        attended, no_shows = _b._rc_db.finalize_no_shows(interaction.guild.id, self.meet_number)
        await _b._rc_refresh_panel(interaction.guild)
        await _b._rc_log_attendance(interaction.guild, self.meet_number, attended, no_shows, interaction.user)
...
# =========================================================
# Reset confirm buttons (no decorators — Pi-compatible)
# =========================================================
class _ConfirmBtn(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await _b._rc_post_new_panel(interaction.guild, ping_roles=True)
        # Re-apply new view after the fresh post
...
class _StaffSelect(discord.ui.Select):
    def __init__(self):
        super().__init__(
            ...
            max_values=1,
            options=[
                discord.SelectOption(label="Finalize Meet 1", value="fin1",
                    description="Mark attendees and flag no-shows for Meet 1."),
                discord.SelectOption(label="Finalize Meet 2", value="fin2",
                    description="Mark attendees and flag no-shows for Meet 2."),
                discord.SelectOption(label="Finalize Meet 3", value="fin3",
                    description="Mark attendees and flag no-shows for Meet 3."),
                discord.SelectOption(label="View Attendance Stats", value="stats",
                    description="See current RSVP counts for all meets."),
                discord.SelectOption(label="Reset Roll Call", value="reset",
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if not isinstance(member, discord.Member) or not any(r.id in _ADMIN_ROLES for r in member.roles):
        ...
        if v == "stats":
            import bot as _b
            embed = _b._rc_build_rollcall_embed(interaction.guild)
...
class RcAdminPatch(commands.Cog, name="RcAdminPatch"):
    ...
    async def refresh_panel(self) -> None:
        ...
        if admin_id:
            try:
                ...
                print("[RcAdminPatch] DB message not found, scanning channel…")
            except Exception as e:
                print(f"[RcAdminPatch] DB edit failed: {e}")

        # Fallback: scan for old 3-button admin panel
        if not bot_id:
            return
        try:
            ...
            print(f"[RcAdminPatch] Scan failed: {e}")
    ...
    async def on_ready(self):
        ...
        print("[RcAdminPatch] Running on_ready refresh…")
