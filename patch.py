...
BOT = "bot.py"
COG = "cogs/diff_announcement_panels.py"

with open(BOT, "r") as f:
    src = f.read()

# Fix gunicorn
src = src.replace('["gunicorn",', '[sys.executable, "-m", "gunicorn",')

# Fix jacket constants
old_leader = re.search(r'LEADER_JACKET = "https://.*?"', src)
if old_leader:
    src = src.replace(old_leader.group(), 'LEADER_JACKET = "diff_data/jackets/jacket_leader.jpeg"')
    print("Fixed LEADER_JACKET")

old_alt = re.search(r'ALT_JACKET = "https://.*?"', src)
if old_alt:
    src = src.replace(old_alt.group(), 'ALT_JACKET = "diff_data/jackets/jacket_alt.jpeg"')
    print("Fixed ALT_JACKET")

crew_block = re.search(r'CREW_JACKETS = \[.*?\]', src, re.DOTALL)
if crew_block:
    new_block = '''CREW_JACKETS = [
    ...
    src = src.replace(crew_block.group(), new_block)
    print("Fixed CREW_JACKETS")

# Fix jacket handler to use discord.File
old_handler = '''        first.set_image(url=LEADER_JACKET)
...
new_handler = '''        leader_fname = os.path.basename(LEADER_JACKET)
...
if old_handler in src:
    ...
    print("Fixed jacket handler")
else:
    print("Jacket handler already patched or not found — skipping")

with open(BOT, "w") as f:
    f.write(src)
print("bot.py saved.")

# Write new announcement cog
cog_content = '''from __future__ import annotations
...
class AnnouncementButtons(discord.ui.View):
    ...
    @discord.ui.button(label="Acknowledge", style=discord.ButtonStyle.success, emoji="✅", custom_id="diff_announce_acknowledge_v1")
    ...
    @discord.ui.button(label="Remind Me", style=discord.ButtonStyle.secondary, emoji="⏰", custom_id="diff_announce_remind_v1")
    async def remind_me(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        _upsert_record(interaction.message.id, record)
        asyncio.create_task(_schedule_dm_reminder(interaction.user, interaction.guild, record["title"], interaction.message.jump_url, LOG_CHANNEL_ID))
        log_ch = interaction.guild.get_channel(LOG_CHANNEL_ID)
        if isinstance(log_ch, discord.TextChannel):
            try:
                ...
                pass
        await interaction.response.send_message(f"⏰ Got it — I\'ll DM you a reminder in **{REMINDER_DELAY_MINUTES} minutes**.", ephemeral=True)

    @discord.ui.button(label="Interested", style=discord.ButtonStyle.primary, emoji="🔥", custom_id="diff_announce_interested_v1")
    ...
    @discord.ui.button(label="View Updates", style=discord.ButtonStyle.link, url=f"https://discord.com/channels/{GUILD_ID}/{VIEW_UPDATES_CHANNEL_ID}", emoji="📍")
...
class CrewAnnouncePanelView(discord.ui.View):
    ...
    @discord.ui.button(label="Make Crew Announcement", emoji="📣", style=discord.ButtonStyle.danger, custom_id="diff_crew_announce_create_v1")
    ...
    @discord.ui.button(label="Refresh Panel", emoji="♻️", style=discord.ButtonStyle.secondary, custom_id="diff_crew_announce_refresh_v1")
...
class GeneralAnnouncementModal(discord.ui.Modal, title="Create General Announcement"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        record = {"title": title, "channel_id": channel.id, "message_id": msg.id, "message_url": msg.jump_url, "created_by": interaction.user.id, "created_at": _utc_now(), "acknowledged": {}, "remind_me": {}, "interested": {}}
...
class GeneralAnnouncePanelView(discord.ui.View):
    ...
    @discord.ui.button(label="Refresh Panel", emoji="♻️", style=discord.ButtonStyle.secondary, custom_id="diff_general_announce_refresh_v1")
...
class AnnouncementPanelsCog(commands.Cog):
    ...
    def _crew_panel_embed(self) -> discord.Embed:
        embed = discord.Embed(title="📢 DIFF Crew Announcement Center", color=discord.Color.dark_red(),
    ...
    def _general_panel_embed(self) -> discord.Embed:
        embed = discord.Embed(title="📢 DIFF General Announcement Center", color=discord.Color.dark_blue(),
    ...
    async def ensure_crew_panel(self):
        await _ensure_panel(self.bot, CREW_ANNOUNCE_CHANNEL_ID, CREW_PANEL_FILE, CREW_PANEL_TAG, self._crew_panel_embed(), self.crew_view, "Crew Announce")

    async def ensure_general_panel(self):
        await _ensure_panel(self.bot, GENERAL_ANNOUNCE_CHANNEL_ID, GENERAL_PANEL_FILE, GENERAL_PANEL_TAG, self._general_panel_embed(), self.general_view, "General Announce")
...
with open(COG, "w", encoding="utf-8") as f:
    f.write(cog_content)
print("Announcement cog updated.")
