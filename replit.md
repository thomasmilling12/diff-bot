...
Panel: My Tickets / Ask Q / Report / Apply / Urgent + Anonymous Tip. Single close entry point `_perform_ticket_close(channel, closer, reason_key)`; `_CSAT_SKIP_REASONS = {"spam","no_response"}`. Claim via topic `ticket_claimed_by=`. 48h warn → 72h auto-close; claimed ticket w/ member's msg unanswered 4h → ping claimer (self-guarding: nudge becomes last msg). Urgent unclaimed 5min → pings support+leadership. Daily 9 AM digest (idempotent) + Monday weekly report (`_ticket_weekly_report_embed`: 7d closes/avg/CSAT/top closers; guard `last_weekly_report_ts` in digest state — helpers are merge-safe `_digest_state_load`). Staff-only 🧠 Catch Me Up button (`diff_ticket_catchup` on `SupportCloseButton`) → ephemeral AI ticket summary. Per-user open lock — `defer()` before acquire. 7-day reopen from CSAT DM. Anonymous tips: Leadership-only reveal, audit-logged. `_ti_*` helpers: close-log ETA, least-busy staff, keyword auto-tag (AI overrides). Join tickets: `JoinTicketView` + Quick Reply presets + micro-bump DM; escalating bumps 2h/6h/24h/72h; claimed apps get one 12h claimer nudge (`claimed_nudge_sent` in join extra state); onboarding DM after accept covers lobby pics + XP (NO roll-call — that's crew-only).
